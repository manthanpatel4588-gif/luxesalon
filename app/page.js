...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @media(max-width:768px){
    ...
    .sidebar{width:100%;height:60px;flex-direction:row;border-right:none;border-top:1px solid var(--border);position:fixed;bottom:0;left:0;right:0;z-index:100;order:2;background:var(--s1)}
    .sb-brand,.sb-footer{display:none}
    .sb-nav{display:flex;flex-direction:row;padding:0 4px;width:100%;justify-content:space-around;align-items:center;gap:0;overflow-y:visible}
...
function VisitModal({ salonId, customers, visit, onClose, onSave, addToast }) {
  ...
  const handleSave = async () => {
    ...
    if (isNew && form.mobile.length !== 10) { addToast('Mobile must be exactly 10 digits', 'error'); return }
        ...
        <div className="modal-body">
          ...
          {!isNew ? (
            ...
            <div className="form-row">
              ...
              <div className="ig">
                <label>Mobile *</label>
                <input placeholder="10-digit mobile" value={form.mobile} maxLength={10} onChange={e => set('mobile', e.target.value.replace(/[^0-9]/g, ''))} />
              </div>
            </div>
          )}
          <div className="ig">
            <label>Services *</label>
            <input placeholder="e.g. Haircut, Blow Dry" value={form.service} onChange={e => set('service', e.target.value)} />
          </div>
          <div className="form-row">
            <div className="ig">
              <label>Amount (₹) *</label>
    ...
    <div>
      ...
      {showModal && (
        <VisitModal salonId={salonId} customers={customers} visit={editVisit}
          onClose={() => setShowModal(false)}
          onSave={() => { setShowModal(false); setEditVisit(null); load() }}
          addToast={addToast}
        />
      )}
    </div>
...
function WhatsApp({ salonId }) {
  ...
  useEffect(() => {
    fetch(`/api/whatsapp?salon_id=${salonId}`)
      .then(r => r.json())
  ...
  const send = c => {
    const msg = encodeURIComponent(`Hello ${c.name}, it's been a while since your last visit. We miss you! Please visit us again soon.`)
...
function LoginPage({ onLogin, toasts }) {
  ...
  const LOGIN_CSS = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    body{background:#0A0A0B;color:#F0EDE8;font-family:'DM Sans',sans-serif;min-height:100vh;overflow:auto}
    .lp{min-height:100vh;display:flex;align-items:center;justify-content:center;background:radial-gradient(ellipse at 20% 50%,rgba(201,168,76,.06) 0%,transparent 60%),radial-gradient(ellipse at 80% 20%,rgba(201,168,76,.04) 0%,transparent 50%);padding:20px}
...
export default function App() {
  ...
  const handleAddVisit = async () => {
    ...
    const data = await res.json()
    setVisitCustomers(Array.isArray(data) ? data : [])
